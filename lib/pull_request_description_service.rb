...
module PullRequestDescriptionService
  class Generator
    ...
    def check_trigger
      if @trigger == "auto" || @client.pull_request(@github_repository, @pr_number).body.include?(@trigger)
        puts "Trigger is #{@trigger}, generating PR description."
      else
        puts "Trigger is #{@trigger}, skipping PR description generation."
        exit
      end
    end
